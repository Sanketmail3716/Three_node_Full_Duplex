...
void Init_CAN_LCD_ADC() {
    ...
    CANSPISetFilter(_CANSPI_FILTER_B2_F3, ID_C, _CANSPI_CONFIG_XTD_MSG);
    CANSPISetFilter(_CANSPI_FILTER_B2_F4, ID_B, _CANSPI_CONFIG_XTD_MSG);
...
void main() {
    ...
    while (1) {
        ...
        CANSPIWrite(ID_B, RxTx_Data, 2, Can_Send_Flags);  // Send to Node B
        Delay_ms(20);
        CANSPIWrite(ID_C, RxTx_Data, 2, Can_Send_Flags); // Send to Node C
        Lcd_Out(1, 1, "NODEA:TXA:");
        Display_Value(adc_value, 1, 12);
        ...
        if (Msg_Rcvd && Rx_Data_Len == 2) {
            ...
            if (Rx_ID == ID_B) {
                Lcd_Out(2, 1, "RXB:");
                Display_Value(received_value, 2, 13);
            } 
           else if (Rx_ID == ID_C) {
                Lcd_Out(2, 9, "RXC:");
                Display_Value(received_value, 2, 5);
            }
