...
void Init_CAN_LCD_ADC() {
    ...
    CANSPISetFilter(_CANSPI_FILTER_B2_F3, ID_A, _CANSPI_CONFIG_XTD_MSG);
...
void main() {
    ...
    while (1) {
        ...
        Lcd_Out(1, 1, "NODEC:RXA:");
        Lcd_Out(2, 9, "TXC:");
        Display_Value(adc_value, 2, 13);
        CANSPIWrite(ID_A, RxTx_Data, 2, Can_Send_Flags);  // Send to Node A
        Delay_ms(20);
        CANSPIWrite(ID_B, RxTx_Data, 2, Can_Send_Flags); // Send to Node C
        ...
        if (Msg_Rcvd && Rx_Data_Len == 2) {
            ...
            if (Rx_ID == ID_A) {
              //  Lcd_Out(1, 9, "RXA:");
                ...
                Lcd_Out(2, 1, "RXB:");
                Display_Value(received_value, 1, 13);
            }
